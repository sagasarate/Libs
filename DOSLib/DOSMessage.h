...
#define LOG_DOS_CHANNEL					11301
#define LOG_DOS_OBJECT_STATE_CHANNEL	11302
#define LOG_DOS_MSG_STATE_CHANNEL		11303
#define LOG_DOS_MEM_STATE_CHANNEL		11304
...
public:
	...
	{
		MSG_LEN_TYPE MsgDataLen = GetDataLength();
		assert(Pos + DataLen <= MsgDataLen);
		if (Pos + DataLen <= MsgDataLen)
		{
...
public:
	...
	bool CloneFrom(LPCVOID pPacket, MSG_LEN_TYPE PacketLen)
	{
		...
		assert(SrcPacketLen == PacketLen);
		if (SrcPacketLen == PacketLen)
		{
			assert(PacketLen <= m_AllocSize);
			if (PacketLen <= m_AllocSize)
			{
	...
	ID_LIST_COUNT_TYPE GetTargetIDCount()
	{
		...
		assert(PacketLen <= m_PacketLen);
		return Count;
	}
	ID_LIST_COUNT_TYPE GetMaxTargetIDCount()
	{
		return (ID_LIST_COUNT_TYPE)((m_AllocSize - sizeof(MSG_LEN_TYPE) - m_Message.GetMsgLength() - sizeof(ID_LIST_COUNT_TYPE)) / sizeof(OBJECT_ID));
	}
	...
	MSG_LEN_TYPE GetMaxMsgDataLen(ID_LIST_COUNT_TYPE MaxTargetIDCount = 1)
	{
		if (MaxTargetIDCount < 1)
			MaxTargetIDCount = 1;
		return m_AllocSize - sizeof(CDOSMessage::DOS_MESSAGE_HEAD) - sizeof(ID_LIST_COUNT_TYPE) - sizeof(OBJECT_ID) * MaxTargetIDCount;
	}
	bool CheckPacket()
	{
