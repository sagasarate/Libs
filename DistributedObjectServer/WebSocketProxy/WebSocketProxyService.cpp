...
void CWebSocketProxyService::Destory()
{
	CDOSObjectProxyServiceDefault::Destory();
	m_pOperator = NULL;
}

bool CWebSocketProxyService::StartService(IDOSObjectProxyServiceOperator* pOperator)
{
	m_pOperator = pOperator;
	m_Config = m_pOperator->GetConfig();
	return CDOSObjectProxyServiceDefault::StartService();
}
...
BOOL CWebSocketProxyService::OnStart()
{
	...
	if (m_Config.EnableBlackList)
	{
		...
		Log(_T("已加载%u个黑名单"), m_IPBlackList.GetObjectCount());
	...
	if (!Create(IPPROTO_TCP,
		...
		false))
	{
		Log(_T("代理服务[%u]创建失败！"), GetID());
		return FALSE;
	}

	if (!StartListen(m_Config.ListenAddress))
	{
		Log(_T("代理服务[%u]侦听于(%s:%u)失败！"),
			GetID(),
			m_Config.ListenAddress.GetIPString(),
			m_Config.ListenAddress.GetPort());
		return FALSE;
	}
	Log(_T("代理服务[%u]侦听于(%s:%u)！"),
		GetID(),
		m_Config.ListenAddress.GetIPString(),
		m_Config.ListenAddress.GetPort());

	Log(_T("对象代理[%u]线程[%u]已启动"), GetID(), GetThreadID());
	...
	{
		LogDebug(_T("代理服务[%u]创建(%u,%u,%u)大小的连接池成功！"),
			GetID(),
	...
	{
		Log(_T("代理服务[%u]创建(%u,%u,%u)大小的连接池失败！"),
			GetID(),
			m_Config.ConnectionPoolSetting.StartSize, m_Config.ConnectionPoolSetting.GrowSize, m_Config.ConnectionPoolSetting.GrowLimit);
		return FALSE;
	}
	...
	if (m_Config.ConnectionGroupCount)
	{
		...
		{
			LogDebug(_T("代理服务[%u]创建(%u,%u,%u)大小的连接销毁队列成功！"),
				GetID(),
				m_Config.ConnectionPoolSetting.StartSize, m_Config.ConnectionPoolSetting.GrowSize, m_Config.ConnectionPoolSetting.GrowLimit);
		}
		else
		{
			Log(_T("代理服务[%u]创建(%u,%u,%u)大小的连接销毁队列失败！"),
				GetID(),
				m_Config.ConnectionPoolSetting.StartSize, m_Config.ConnectionPoolSetting.GrowSize, m_Config.ConnectionPoolSetting.GrowLimit);
			return FALSE;
		}
		...
		LogDebug(_T("代理服务[%u]创建了%u个连接组线程"), GetID(), (UINT)m_ConnectionGroups.GetCount());
		for (UINT i = 0; i < m_ConnectionGroups.GetCount(); i++)
		{
			if (!m_ConnectionGroups[i].Init(this, m_Config, i))
				return FALSE;
			m_ConnectionGroups[i].Start();
			LogDebug(_T("连接组线程[%u]已启动"), m_ConnectionGroups[i].GetThreadID());
		}
	}
...
CBaseNetConnection* CWebSocketProxyService::CreateConnection(CIPAddress& RemoteAddress)
{
	...
	if (pConnection)
	{
		if (!pConnection->Init(this, m_Config, ID))
		{
			...
			pConnection = NULL;
			LogDebug(_T("现有连接数%u"), m_WSConnectionPool.GetObjectCount());
		}
	}
	return pConnection;
}
...
int CWebSocketProxyService::CheckFreeObject()
{
	...
	if (ProcessCount)
	{
		LogDebug(_T("已彻底释放%d个连接"), ProcessCount);
	}
	return ProcessCount;
}