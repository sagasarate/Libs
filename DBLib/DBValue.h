...
namespace DBLib
{
	...
	public:
		...
		void SetValue(int ValueType, LPCVOID pData, size_t DataSize, UINT DitigalSize);
		void SetEmptyValue(int ValueType, size_t DataSize, UINT DitigalSize);
		void SetNULLValue(int ValueType);

		LPVOID GetBuffer();
		int GetType();
		size_t GetLength();
		UINT GetDigitalLength();
		bool IsNull();
