...
namespace DBLib
{
	...
	int CDBFakeRecordSet::Init(LPCVOID pData, size_t DataSize)
	{
		Destory();
	...
	size_t CDBFakeRecordSet::GetRecordSetDataSize()
	{
	...
	int CDBFakeRecordSet::GetRecordSetDataEx(size_t& Pos, LPVOID pBuffer, size_t& BufferSize)
	{
		int ColNum = 0;
		size_t PeekPtr = 0;
		...
		*pRecordNum = 0;
		...
		while (PeekPtr < m_DataBuffer.GetUsedSize())
		{
			...
			if (RecordPtr + RecordLen - Pos > OutputBuffer.GetFreeSize())
				break;
			RecordPtr += RecordLen;
			(*pRecordNum)++;
	...
	bool CDBFakeRecordSet::Close()
	{
		return false;
	}
	bool CDBFakeRecordSet::SetBlobMaxProcessSize(size_t MaxSize)
	{
		return false;
	}
}
