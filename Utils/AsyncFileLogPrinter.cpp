...
void CAsyncFileLogPrinter::DoLogBackup()
{
	if (m_LogBackupDelay)
	{
		...
		UINT Delay = abs(m_LogBackupDelay) * 24 * 60 * 60;
		...
		while (FileSearcher.FindNext())
		{
			...
			if (FileTime + Delay < CurTime)
			{
				...
				{
					//删除文件
					if (CFileTools::DeleteFile(FileSearcher.GetFilePath()))
					{
						PrintImportantLog(_T("删除文件成功:%s"), (LPCTSTR)FileSearcher.GetFilePath());
					}
					else
					{
						PrintImportantLog(_T("删除文件失败:%s"), (LPCTSTR)FileSearcher.GetFilePath());
					}
				}
			}
		}
