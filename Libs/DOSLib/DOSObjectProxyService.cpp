...
BOOL CDOSObjectProxyService::OnStart()
{
	...
	{
		PrintDOSLog(0xff0000,_T("代理服务[%u]创建%u大小的连接池失败！"),
			GetID(),
	...
	{
		PrintDOSLog(0xff0000,_T("代理服务[%u]创建%u大小的消息队列失败！"),
			GetID(),
	...
	if (m_Config.MinMsgCompressSize)
	{
		...
		case MSG_COMPRESS_LZO:
			...
			break;
		default:
			PrintDOSLog(0xff0000, _T("代理服务[%u]不支持消息压缩模式%u！"),
	...
	if(!Create(IPPROTO_TCP,
		...
		false))
	{
		PrintDOSLog(0xffff, _T("代理服务[%u]创建失败！"), GetID());
		return FALSE;
	}

	if (!StartListen(m_Config.ListenAddress))
	{
		PrintDOSLog(0xffff,_T("代理服务[%u]侦听于(%s:%u)失败！"),
			GetID(),
			m_Config.ListenAddress.GetIPString(),
			m_Config.ListenAddress.GetPort());
		return FALSE;
	}
	PrintDOSLog(0xffff,_T("代理服务[%u]侦听于(%s:%u)！"),
		GetID(),
		m_Config.ListenAddress.GetIPString(),
		m_Config.ListenAddress.GetPort());

	PrintDOSLog(0xff0000,_T("对象代理[%u]线程[%u]已启动"),GetID(),GetThreadID());
	return TRUE;
	FUNCTION_END;
	return FALSE;
}
