...
#include "StdAfx.h"        // DirectShow base class definitions
...
{
    CComAutoLock cObjectCreationLock(&m_ObjectCreationLock);
    if (m_pPosition) {
...
{
    CComAutoLock cRendererLock(&m_InterfaceLock);
...
BOOL CBaseRenderer::ScheduleSample(IMediaSample *pMediaSample)
{
	...
	//PrintSystemLog(0,"当前采样时间%d",StartSample/10000000);
...
{
    CComAutoLock cRendererLock(&m_RendererLock);
...
{
    CComAutoLock cInterfaceLock(&m_InterfaceLock);
    m_bInReceive = TRUE;
    ...
    CComAutoLock cSampleLock(&m_RendererLock);
...
HRESULT CBaseRenderer::Receive(IMediaSample *pSample)
{
	...
	REFERENCE_TIME StartSample, EndSample;
	hr = GetSampleTimes(m_pMediaSample, &StartSample, &EndSample);
	if (SUCCEEDED(hr)) 
	{
		if(StartSample<m_RecentSampleTime)
		{
#ifdef _DEBUG
			PrintSystemLog(0,"强制渲染");
#endif
...
{
    CComAutoLock cRendererLock(&m_pRenderer->m_InterfaceLock);
    CComAutoLock cSampleLock(&m_pRenderer->m_RendererLock);
    ...
    if (FAILED(hr)) {
...

        {
            ...
            if (!IsStopped() && !IsFlushing() && !m_pRenderer->m_bAbort && !m_bRunTimeError) {
                ...
                {
                    CComAutoLock alRendererLock(&m_pRenderer->m_RendererLock);
...
STDMETHODIMP CBaseVideoRenderer::get_FramesDroppedInRenderer(int *pcFramesDropped)
{
    ...
    CComAutoLock cVideoLock(&m_InterfaceLock);
