...
void WINAPI DbgBreakPoint(const TCHAR *pFileName,INT iLine,const TCHAR* szFormatString,...)
{
    ...
    int nReturnValue = _vsntprintf_s( szBreakPointMessage, MAX_BREAK_POINT_MESSAGE_SIZE,MAX_CHARS_IN_BREAK_POINT_MESSAGE, szFormatString, va );
...
void WINAPI DbgLogInfo(DWORD Type,DWORD Level,const TCHAR *pFormat,...)
{
    ...
    _vstprintf_s(szInfo + lstrlen(szInfo), 2000-lstrlen(szInfo),pFormat, va);
    lstrcat(szInfo, TEXT("\r\n"));
    DbgOutString(szInfo);
...
{
#ifdef DEBUG
    _stprintf_s(m_String,50, TEXT("%.16g"), d);
#else
