...
void CNTService :: RegisterApplicationLog( LPCTSTR lpszFileName, DWORD dwTypes ) {
	TCHAR szKey[256];
	_tcscpy_s(szKey,256, gszAppRegKey);
	_tcscat_s(szKey,256, m_lpServiceName);
	HKEY hKey = 0;
	LONG lRet = ERROR_SUCCESS;
 		...
 		if( lRet == ERROR_SUCCESS ) {
			...
			if( lRet == ERROR_SUCCESS ) {
				...
				if( ! * p ) {
					...
					// too.
					_tcscpy_s(p,dwNewSize, m_lpServiceName);
...
void CNTService::OnSetServiceName()
{
