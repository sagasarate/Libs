...
void CHashMD5::AddData(BYTE * inBuf, size_t inLen)
{
	...
	if ((m_mdContext.i[0] + ((UINT4)inLen << 3)) < m_mdContext.i[0])
	m_mdContext.i[1]++;
	...
	while (inLen--)
	{
		...
		if (mdi == 0x40)
		{
			...
			in[i] = (((UINT4)m_mdContext.in[ii+3]) << 24) |
					(((UINT4)m_mdContext.in[ii+2]) << 16) |
					(((UINT4)m_mdContext.in[ii+1]) << 8) |
...
void CHashMD5::MD5Final(void)
{
	...
	for (i = 0, ii = 0; i < 4; i++, ii += 4)
	{
		...
		m_mdContext.digest[ii+1] =
			(unsigned char)((m_mdContext.buf[i] >> 8) & 0xFF);
		m_mdContext.digest[ii+2] =
			(unsigned char)((m_mdContext.buf[i] >> 16) & 0xFF);
		m_mdContext.digest[ii+3] =
			(unsigned char)((m_mdContext.buf[i] >> 24) & 0xFF);
	}
}

void CHashMD5::Transform(UINT4 * buf, UINT4 * in)
{
...
CEasyString CHashMD5::GetHashCodeString(void)
{
	CEasyString HashCode,temp;
	int i;
