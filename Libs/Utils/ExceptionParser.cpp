...
{
	PrintImportantLog(0xff,_T("收到未处理的异常"));
...
{
	PrintImportantLog(0xff,_T("捕捉到异常,Code=%u"),Code);
	CExceptionParser::GetInstance()->ParseException(pException);	
}

LONG CExceptionParser::ExceptionPrinter(LPEXCEPTION_POINTERS pException,UINT64 DebugInfo1,LPCTSTR szFunName)
{
	PrintImportantLog(0xff,_T("开始输出异常,DebugInfo1=0x%llX,在函数%s"),DebugInfo1,szFunName);
	CExceptionParser::GetInstance()->ParseException(pException);
...
void CExceptionParser::Init(UINT Flag)
{
	...
	{
		PrintImportantLog(0xff,_T("启用未处理异常捕捉"));
...
void CExceptionParser::ParseException(LPEXCEPTION_POINTERS pException)
{
	...
	PrintImportantLog(0xff,_T("进行异常处理"));

	PROCESS_MEMORY_COUNTERS MemoryInfo;
	if(GetProcessMemoryInfo(m_hProcess,&MemoryInfo,sizeof(MemoryInfo)))
	{
#ifdef _WIN64
		PrintImportantLog(0xff,_T("进程内存占用:%llu,虚拟内存占用:%llu"),MemoryInfo.WorkingSetSize,MemoryInfo.PagefileUsage);
#else
		PrintImportantLog(0xff,_T("进程内存占用:%u,虚拟内存占用:%u"),MemoryInfo.WorkingSetSize,MemoryInfo.PagefileUsage);
#endif		
	}
	else
	{
		PrintImportantLog(0xff,_T("获取进程内存使用情况失败%d"),GetLastError());
	}
...
BOOL CExceptionParser::WriteDump(LPEXCEPTION_POINTERS pException)
{
	...
	PrintImportantLog(0xff,_T("开始写入%sDump文件%s"),
	...
	{		
		...
		PrintImportantLog(0xff,_T("创建Dump文件成功"));
		...
		{
			PrintImportantLog(0xff,_T("写入Dump文件成功%s"),szDumpFileName);
			CloseHandle(hDumpFile);
			return TRUE;
		}
		else
		{
			PrintImportantLog(0xff,_T("写入Dump文件失败%s"),szDumpFileName);
		}
		CloseHandle(hDumpFile);
	...
	{
		PrintImportantLog(0xff,_T("打开Dump文件失败%s"),szDumpFileName);
	}
...
BOOL CExceptionParser::SymInit()
{
	...
	PrintImportantLog(0xff, _T("dbghelp.dll版本%d.%d.%d.%d,路径:%s"),
	...
	PrintImportantLog(0xff, _T("搜索符号文件在:%s"), szModulePath);
	...
	PrintImportantLog(0xff,_T("初始化符号完毕"));
...
{
	PrintImportantLog(0xff,_T("开始从%s加载符号"),szModuleFileName);
	...
	if(hModule)
	{
		...
		{
			PrintImportantLog(0xff,_T("模块起始地址:0x%llX"),(DWORD64)ModuleInfo.lpBaseOfDll);
			if(SymLoadModuleExT(m_hProcess,NULL,szModuleFileName,NULL,(DWORD64)ModuleInfo.lpBaseOfDll,0,NULL,0))
			{
				PrintImportantLog(0xff,_T("加载符号成功"));
				return TRUE;
			}
			else
			{
				PrintImportantLog(0xff,_T("加载符号失败%d"),GetLastError());			
			}
		}
		else
		{
			PrintImportantLog(0xff,_T("获取模块信息失败%d"),GetLastError());
		}
	}
	else
	{
		PrintImportantLog(0xff,_T("获取模块句柄失败%d"),GetLastError());
	}
	return FALSE;
}
...
{
	PrintImportantLog(0xff, _T("开始枚举模块符号加载状态"));
	if (!SymEnumerateModules64(hProcess, EnumModulesCallback, hProcess))
	{
		PrintImportantLog(0xff, _T("枚举模块失败%d"), GetLastError());
	}
	PrintImportantLog(0xff, _T("枚举模块符号加载状态完毕"));
}