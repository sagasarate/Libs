...
bool CDialogItemSorter::AddDialogItem(UINT ParentID, UINT ID, int SortType, CWnd* pDialogItem, CRect Rect, float SortParam1, float SortParam2)
{
	SORT_TREE_ITEM* pParent = FindTreeNode(m_pItemTree, ParentID);
	if (pParent)
	{
		...
		pNewNode->SortParam1 = SortParam1;
		pNewNode->SortParam2 = SortParam2;
		pNewNode->pDialogItem = pDialogItem;
		pNewNode->Rect = Rect;
...
void CDialogItemSorter::GetDialogItemSize(SORT_TREE_ITEM* pRoot, bool GetRoot)
{
	if (GetRoot)
	{
		CWnd* pDialogItem = pRoot->pDialogItem;
		if (pDialogItem == NULL)
			pDialogItem = m_pParent->GetDlgItem(pRoot->DialogItemID);
		if (pDialogItem)
